...
export default function OrderTracking() {
  ...
  const getInitialStep = () => {
    if (!order) return 0
    const idx = STEPS.findIndex(s => s.id === order.status.toLowerCase())
    return idx === -1 ? 0 : idx
  }
      ...
      <div className="page">
        <div className={styles.hero}>
          <div className="container">
            ...
            <p className={styles.orderId}>Order #{orderId.replace('o', '')}</p>
          </div>
        </div>
            ...
            {order?.status === 'cancelled' ? (
              ...
              <div className={styles.statusHeader}>
                <div className={styles.estTime}>
                  ...
                  <span className={styles.estValue}>{Math.max(5, estimatedTime - (currentStep * 6))} mins</span>
                </div>
                    ...
                    <button 
                      ...
                      onClick={() => {
                        order?.items.forEach(item => {
                          addItem({ _id: item.menuId, name: item.name, price: item.price, image: 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?auto=format&fit=crop&w=100' })
                        })
                        navigate('/cart')
                      }}
                    >
